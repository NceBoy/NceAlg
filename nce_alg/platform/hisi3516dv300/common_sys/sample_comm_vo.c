		...
		for (i = 0; i < u32WndNum; i++) {
			if( enMode == VO_MODE_1MUX  ||
				...
				if(u32Width > u32Height) { //横屏
					...
					/*
					if(u32Square >= 2) {
						//固定为4:3显示  如 (800-1280/2*3/4)/2
						stChnAttr.stRect.s32X   	= ALIGN_DOWN((u32Width-(((u32Height / u32Square)*3)/4))/2, 2);
						stChnAttr.stRect.s32Y   	= ALIGN_DOWN((u32Height / u32Square) * (i % u32Square), 2);
						stChnAttr.stRect.u32Width   = ALIGN_DOWN((((u32Height / u32Square)*3)/4), 2);
