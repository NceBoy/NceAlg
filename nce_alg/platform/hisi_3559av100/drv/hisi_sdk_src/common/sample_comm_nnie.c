    ...
    SAMPLE_SVP_NNIE_PARAM_S *pstNnieParam)
{
	...
	//u32NetSegNum Ϊ����ģ����NNIEִ�е�����ֶ�����ȡֵΪ1~8��
	//����ķֶ���ָģ��ִ���п��ܻ�ֳɶ�Σ�һЩ����NNIE��ִ�У�һЩ����CPU��DSP��ִ�С�
	//ÿ���ν������������ֵ��pstNnieParam��Ӧ�Ķ���
	for(i = 0; i < pstNnieParam->pstModel->u32NetSegNum; i++)
	{
		//printf("type = %d\n",pstNnieParam->pstModel->astSeg[i].enNetType);
	    ...
	    {
	    	//printf("src j =%d :n = %d,w=%d,h=%d\n",j, pstNnieParam->pstModel->astSeg[i].astSrcNode[j].unShape.stWhc.u32Chn,
			...
			if(pstNnieParam->p_algname && strcmp(pstNnieParam->p_algname,"mtcnn_0") == 0)
			{
				printf("change reslo !\n");
				pstNnieParam->pstModel->astSeg[i].astSrcNode[j].unShape.stWhc.u32Width  = 640;
				pstNnieParam->pstModel->astSeg[i].astSrcNode[j].unShape.stWhc.u32Height = 480;
			}
		...
		{
			//printf("des j =%d :n = %d,w=%d,h=%d\n",j, pstNnieParam->pstModel->astSeg[i].astDstNode[j].unShape.stWhc.u32Chn,
			...
			//printf("node name = %s\n",pstNnieParam->pstModel->astSeg[0].astDstNode[j].szName);

			if(pstNnieParam->p_algname &&strcmp(pstNnieParam->p_algname,"mtcnn_0") == 0)
			{
				printf("change reslo !\n");
				pstNnieParam->pstModel->astSeg[i].astDstNode[j].unShape.stWhc.u32Width	= 640/2;
				pstNnieParam->pstModel->astSeg[i].astDstNode[j].unShape.stWhc.u32Height = 480/2;
			}
	...
	HI_U32 u32TotalStep,SVP_BLOB_S astBlob[], HI_U32 u32Align, HI_U32* pu32TotalSize,HI_U32 au32BlobSize[])
{
	...
	for(i = 0; i < u32NodeNum; i++)
	{
		...
		*pu32TotalSize += au32BlobSize[i];
		...
		//printf("str = %d,type = %d,w= %d\n",u32Stride,astNnieNode[i].enType,astNnieNode[i].unShape.stWhc.u32Width);
	}
}
...
HI_S32 SAMPLE_COMM_SVP_NNIE_LoadModel_(char *p_model, int model_size,
    SAMPLE_SVP_NNIE_MODEL_S *pstNnieModel)
{
	HI_S32 s32Ret = HI_INVALID_VALUE;
	HI_U64 u64PhyAddr = 0;
	...
	memcpy(pu8VirAddr,p_model,slFileSize);
