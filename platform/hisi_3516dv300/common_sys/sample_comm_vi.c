...
combo_dev_attr_t MIPI_2lane_CHN0_SENSOR_GC2093_12BIT_27M_WDR2TO1_ATTR =
{
	.devno = 0,
    .input_mode = INPUT_MODE_MIPI,
    .data_rate = MIPI_DATA_RATE_X1,
...
//added gc2093 by rpdzkj lixin
VI_DEV_ATTR_S DEV_ATTR_GC2093_27M_BASE = 
{
	VI_MODE_MIPI,
    VI_WORK_MODE_1Multiplex,
    {0xFFF00000,    0x0},
...
//            break;
		...
		case GALAXYCORE_GC2093_MIPI_27M_30FPS_12BIT:
			hi_memcpy(pstComboAttr, sizeof(combo_dev_attr_t), &MIPI_2lane_CHN0_SENSOR_GC2093_12BIT_27M_WDR2TO1_ATTR, sizeof(combo_dev_attr_t));
			SAMPLE_PRT("sample_comm_vi select GALAXYCORE_GC2093_MIPI_27M_30FPS_12BIT=============!\n");
            break;
...
HI_S32 SAMPLE_COMM_VI_GetDevAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_DEV_ATTR_S* pstViDevAttr)
{
    switch (enSnsType)
    {
		...
		//added gc2093 by rpdzkj lixin
		 case GALAXYCORE_GC2093_MIPI_27M_30FPS_12BIT:
            hi_memcpy(pstViDevAttr, sizeof(VI_DEV_ATTR_S), &DEV_ATTR_GC2093_27M_BASE, sizeof(VI_DEV_ATTR_S));
			SAMPLE_PRT("sample_comm_vi SAMPLE_COMM_VI_GetDevAttrBySns==============!\n");
...
HI_S32 SAMPLE_COMM_VI_GetPipeAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_PIPE_ATTR_S* pstPipeAttr)
{
    switch (enSnsType)
    {
		...
		case GALAXYCORE_GC2093_MIPI_27M_30FPS_12BIT:
            hi_memcpy(pstPipeAttr, sizeof(VI_PIPE_ATTR_S), &PIPE_ATTR_1920x1080_RAW10_420_3DNR_RFR, sizeof(VI_PIPE_ATTR_S));
			printf("sample_comm_vi SAMPLE_COMM_VI_GetPipeAttrBySns ==================\n");
            break;
...
HI_S32 SAMPLE_COMM_VI_GetChnAttrBySns(SAMPLE_SNS_TYPE_E enSnsType, VI_CHN_ATTR_S* pstChnAttr)
{
    switch (enSnsType)
    {
		...
		case GALAXYCORE_GC2093_MIPI_27M_30FPS_12BIT:
			hi_memcpy(pstChnAttr, sizeof(VI_CHN_ATTR_S), &CHN_ATTR_1920x1080_420_SDR8_LINEAR, sizeof(VI_CHN_ATTR_S));
			printf("sample_comm_vi SAMPLE_COMM_VI_GetChnAttrBySns ==============!\n");
            break;
    ...
    for (i = 0; i < WDR_MAX_PIPE_NUM; i++)
    {
        if (pstViInfo->stPipeInfo.aPipe[i] >= 0  && pstViInfo->stPipeInfo.aPipe[i] < VI_MAX_PIPE_NUM)
        {
            ...
            if (bNeedChn)
            {
                ...
                {
					//add rotation_90 by rpdzkj lixin
					/*if(g_mipi_tx_lcd_resolution->pu32W < g_mipi_tx_lcd_resolution->pu32H){
						
						s32Ret = HI_MPI_VI_SetChnRotation(ViPipe,ViChn,ROTATION_90);
						if(s32Ret != HI_SUCCESS)
						{
							SAMPLE_PRT("Rotation_90 failed with %#x!\n", s32Ret);
							return HI_FAILURE;
						}
						SAMPLE_PRT("Rotation_90 successful!\n", s32Ret);
					}*/
